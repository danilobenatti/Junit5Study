...
//		listaImports.add("import java.util.ArrayList;");
	...
	public void gerarCodigoClasse(Class classe) {
		...
		builder.append("\tprivate ").append(classe.getSimpleName())
			.append(" elemento;\n");
		
		builder.append("\tprivate ").append(nomeClasse).append("(){}\n\n");
		
		builder.append("\tpublic static ").append(nomeClasse).append(" um")
			.append(classe.getSimpleName()).append("() {\n");
		builder.append("\t\t").append(nomeClasse).append(" builder = new ")
			.append(nomeClasse).append("();\n");
		builder.append("\t\tinicializarDadosPadroes(builder);\n");
		builder.append("\t\treturn builder;\n");
		builder.append("\t}\n\n");
		
		builder.append("\tpublic static void inicializarDadosPadroes(")
			.append(nomeClasse).append(" builder) {\n");
		builder.append("\t\tbuilder.elemento = new ")
			.append(classe.getSimpleName()).append("();\n");
		builder.append("\t\t").append(classe.getSimpleName())
			.append(" elemento = builder.elemento;\n");
		builder.append("\n\t\t\n");
		...
		for (Field campo : declaredFields) {
			...
			if (campo.getType().getSimpleName().equals("List")) {
				ParameterizedType stringListType = (ParameterizedType) campo
					.getGenericType();
				builder.append("\tpublic ").append(nomeClasse)
					.append(" comLista")
					.append(campo.getName().substring(0, 1).toUpperCase())
					.append(campo.getName().substring(1)).append("(")
					.append(((Class) stringListType.getActualTypeArguments()[0])
						.getSimpleName())
					.append("... params) {\n");
...
////				elemento.setelemTypes(lista);
				builder.append("\t\telemento.set")
					.append(campo.getName().substring(0, 1).toUpperCase())
					.append(campo.getName().substring(1))
				...
				builder.append("\tpublic ").append(nomeClasse).append(" com")
					.append(campo.getName().substring(0, 1).toUpperCase())
					.append(campo.getName().substring(1)).append("(")
					.append(campo.getType().getSimpleName())
					.append(" param) {\n");
		...
		builder.append("\tpublic ").append(classe.getSimpleName())
			.append(" agora() {\n");
